              ...
              <div class="p-3 bg-white">
                <div v-if="cart.final_total > 0">
                  <p class="py-3 text-center border-bottom border-primary">
                    <span class="fw-bold">{{ cart.carts.length }}</span> 項商品</p>
                  <ul class="list-unstyled">
                    <li v-for="item in cart.carts" :key="item.product_id" class="py-3 border-bottom border-default">
                      <div class="row">
                        <div class="col-4">
                  ...
                  <RouterLink to="/cart"
                    class="py-2 btn btn-primary text-center rounded-0 w-100 fw-bold">前往購物車</RouterLink>
                </div>
                <div v-else>
                  <p class="py-3 text-center">目前購物車內沒有內容</p>
                  <RouterLink to="/products" class="py-2 btn btn-primary text-center rounded-0 w-100 fw-bold">瀏覽商品</RouterLink>
                </div>
              </div>
            </ul>
          </div>
